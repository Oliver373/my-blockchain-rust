...
impl Blockchain {
    ...
    pub fn update_with_block (&mut self, block: Block) -> Result<(),BlockValidationErr> {
        ...
        if block.index != i as u32 {
            ...
            println!("Diffifulty fail");
            return Err(BlockValidationErr::InvalidHash);
        } else if i != 0 {
            let prev_block = &self.blocks[i-1];
            if block.timestamp <= prev_block.timestamp {
                println!("Time did not increase index: {}", &block.index);
                return Err(BlockValidationErr::AchronologicalTimestamp);
            } else if block.prev_block_hash != prev_block.hash {
                println!("Hash mismach index index: {}", &block.index);
                return Err(BlockValidationErr::MismatchedPreviouHash);
            }
        } else {
        ...
        if let Some((coinbase,transactions)) = block.transactions.split_first() {
            ...
            for tansaction in transactions {
                let input_hashes = tansaction.input_hashes();

                if 
                    !(&input_hashes - &self.unspent_outputs).is_empty() || 
                    !(&input_hashes & &block_spent).is_empty()
                {
                    return  Err(BlockValidationErr::InvaildInput);
                }

                let input_value = tansaction.input_value();
                let output_value = tansaction.output_value();
                ...
                block_created.extend(tansaction.output_hashes())
